...
def addNewNames(urlNames, keyDF, newRatio=0.5,language='fr', limitCount=9):
    ...
    if(not newDF.empty):
      for index, column in newDF.iterrows():
        ...
        if(not ( in keyDF['keyword'].unique()):
...
keywordsDF = addNewNames('https://raw.githubusercontent.com/pg-ufr-news/gensChuchoter/main/csv/sentiments_new_persons.csv', keywordsDF, 0.6, 'fr', 9)
