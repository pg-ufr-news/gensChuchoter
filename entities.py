...
for index, column in newsDf.iterrows():
    ...
    newsDf.loc[newsDf['url'] == column['url'], 'subjectivity'] = subjectivity
    newsDf.loc[newsDf['url'] == column['url'], 'sentiment'] = polarity
    try:
        ...
        for entity in doc.ents:
            ...
            if(entity.label_ in ['LOC','GPE']):
                if(entity.text in indexLocations):
                    ...
                    indexLocations[entity.text]['sentiment'] += polarity
                    indexLocations[entity.text]['subjectivity'] += subjectivity
                else:      
       
                    indexLocations[entity.text] = {'phrase':entity.text, 'label':entity.label_, 'sentiment':polarity,
                                                   'subjectivity':subjectivity, 'language':lang,'count':1}
             ...
             if(strangeCharacters(personText,".,!?;:'…<>/\n\r")==0):
               if(personText.count(' ')>0):
                if(personText in indexPersons):
                    ...
                    indexPersons[personText]['sentiment'] += polarity
                    indexPersons[personText]['subjectivity'] += subjectivity
                else:    
                    indexPersons[personText] = {'phrase':personText, 'label':entity.label_, 'sentiment':polarity,
                                                 'subjectivity':subjectivity, 'language':lang, 'count':1} 
                if(not personInSearch(personText)):
                  if(personText in indexNewPersons):
                    indexNewPersons[personText]['count'] += 1
                    indexNewPersons[personText]['sentiment'] += polarity
                    indexNewPersons[personText]['subjectivity'] += subjectivity
                  else:    
                    indexNewPersons[personText] = {'phrase':personText, 'label':entity.label_, 'sentiment':polarity,
                                                 'subjectivity':subjectivity, 'language':lang, 'count':1}   
            elif('ORG' == entity.label_):
                if(entity.text in indexOrganizations):
                    indexOrganizations[entity.text]['count'] += 1
                    indexOrganizations[entity.text]['sentiment'] += polarity
                    indexOrganizations[entity.text]['subjectivity'] += subjectivity
                else:    
                    indexOrganizations[entity.text] = {'phrase':entity.text, 'label':entity.label_, 'sentiment':polarity,
                                                       'subjectivity':0, 'language':lang, 'count':1} 
            elif('MISC' == entity.label_):
                if(entity.text in indexMisc):
                    indexMisc[entity.text]['count'] += 1
                    indexMisc[entity.text]['sentiment'] += polarity
                    indexMisc[entity.text]['subjectivity'] += subjectivity
                else:         
                    indexMisc[entity.text] = {'phrase':entity.text, 'label':entity.label_, 'sentiment':polarity,
                                              'subjectivity':subjectivity, 'language':lang, 'count':1} 
            else:
                if(entity.text in indexMissing):
                    indexMissing[entity.text]['count'] += 1
                    indexMissing[entity.text]['sentiment'] += polarity
                    indexMissing[entity.text]['subjectivity'] += subjectivity
                else:
                    indexMissing[entity.text] = {'phrase':entity.text, 'label':entity.label_, 'sentiment':polarity,
                                                 'subjectivity':subjectivity, 'language':lang, 'count':1}  
